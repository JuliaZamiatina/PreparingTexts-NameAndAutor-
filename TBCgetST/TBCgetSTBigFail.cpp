...
//
	{
		...
		volatile int NNF[1000], NNL[4000], NLL[1000], nF, nL, tb4, tb5, tb20, nf1, nt1;
		...
		int jA = 0, jz = 0, vZ = 0;//NT9, h1, h2, JS, 
		char ANm[1001] = { "" };
		//char SOAUT[20] = "Соавтор_";
		int nOfAs = 1;
...
//
...

		{
			...
			if (j > 0)
			{
				...
				for (x = 0; x < NT10; x++)
				{
					...
					if ((p == 0) && (Buf[j + 1] == '\0')) { FileNameNumber = t2; break; }
					else k = t2;
				...
				if (FileNameNumber == 0) {
					...
					for (x = 0; x < Ndx; x++)
					{
						...
						if ((p == 0) && (s[w + j + 1] == '\0')) { FileNameNumber = NbF[x]; break; }
						w = w + 260;
					}
...

		{
			...
			fopen_s(&f1, nmT, "r");  //текст
			nOfAs = FileNameNumber = j = NNN = p = 0; c1 = c2 = '\0';
			for (x = 0; x < 1001; x++)ANm[x] = '\0';
			...
			if (j > 0)
			{
				if (nOfAs == 0)
				{
					...
					for (x = 0; x < NT10; x++)
					{
						...
						if (p == 1)
							k = t2; //предполагается упорядоченность номеров по возрастанию
						else if ((p == 0) && (Buf[i] == '\0')) 
						{ 
							FileNameNumber = t2; 
							break; 
					...
					if (FileNameNumber == 0) {
						...
						{
							c = ANm[0]; p = 0; i = 0;
							while (c != '\0') { if (c != s[w + i]) { p = 1; break; } else { i++; c = ANm[0]; } }
							if ((p == 0) && (s[w + i + 1] == '\0')) { FileNameNumber = NbF[x]; break; }
							w = w + 100;
						}
					}
					if (FileNameNumber == 0) {
						...
						fprintf_s(f2, "\"%s\";", ANm);
						fprintf_s(f2, "%d;0\n", k);
						if (Nd10 < 5000) NbF[Nd10] = k;
						Nd10++;
						c = ANm[0]; p = 0; i = 0;
						while (c != '\0'&& i < 1000) { s[d10 + i] = c; i++; c = ANm[i]; }
						s[d10 + i] = c;
						if (Nd10 < 1921)d10 = d10 + 260;
					}
				...
				else//Несколько авторов
				{
					...
					for (i = 0; i < 1000; i++)
						NNF[i] = 0;
					//Выделяем автора без начальных пробелов в NameOfText
					for (jz = 0; jz < nOfAs + 1; jz++) {
						c = ANm[jA];
						i = p = 0;
						while (c != ','&& c != '\0') {
							if (c != ' ') p = 1;
						...
						for (x = 0; x < NT10; x++)
						{
							...
							p = 0; i = 0;
							while (NameOfText[i] != '\0')
							{ 
								if (NameOfText[i] != Buf[i])
								{ 
									p = 1; 
									break; 
								} 
							...
							if (p == 1) k = t2; //предполагается упорядоченность номеров по возрастанию
							if ((p == 0) && (Buf[i] == '\0')) { FileNameNumber = t2; }
							else k = t2;
						}
						fclose(f1);

						// а в новой таблице авторов?
						if (FileNameNumber == 0)
						{
							if (Nd10 < 5000) Ndx = Nd10; else Ndx = 4999;
						...
						if (FileNameNumber == 0)
						{
							...
							while ((NameOfText[i] != '\0') && (i < 255))
							{ 
								s[d10 + i] = NameOfText[i];
								i++; 
							}
							s[d10 + i] = NameOfText[i];
							if (Nd10 < 1921)d10 = d10 + 260;
						}
						NNF[jz] = FileNameNumber;
					}//Конец цикла по авторам
					...
					//если все авторы уже были
					if (vZ == 0)//поиск списка соавторов для существующих авторов
					{
						fopen_s(&f4, "C:\\Tbase\\t9.txt", "r");  //файл существующих соавторов
						while ((c = fgetc(f4)) != '\n'); //Пропуск имен

					//	h1 = 0; h2 = 0;

						FileNameNumber = 0; x = 0; z = 0; i = 0; m = 0;

						while (!feof(f4)) {
							fscanf_s(f4, "%*d;%d;%d\n", &x, &i);
							//если номер соавтора совпадает с предыдущим, читаем дальше
							if (((x == z) ^ (m == 0)) && (i == NNF[m])) m++;
							z = x;
							if (m == nOfAs + 1) { FileNameNumber = x; break; }
						}
						fclose(f4);
						//проверка по буферу
						//int y = 0;
						...
						if (FileNameNumber == 0) {
							...
							//в буфер
							if (NNL[0] < 1921) SF[NNL[0]] = FileNameNumber; else SF[1920] = FileNameNumber;
							NNL[nL + 1] = nOfAs + 1;
							for (x = 0; x < nOfAs + 1; x++) {
								NNL[nL + x + 2] = NNF[x];
							}
					...
					else
					{
						...
						//	}
						//	//в буфер
						//	if (nL < 10000) NNF[nL / 5] = FileNameNumber;
						//	for (x = 0; x < nOfAs; x++) {
						//		NLL[nL + x] = SAutorsNumb[x];
						//	}
						//	nL += 5;
						//}
						//}
